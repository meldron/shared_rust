...
#if canImport(normalizerFFI)
import normalizerFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_normalizer_c9ab_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_normalizer_c9ab_rustbuffer_free(self, $0) }
    }
}
...
public func normalizeUsername(s: String )  -> String {
    let _retval = try!
    
    
    rustCall() {
    
    normalizer_c9ab_normalize_username(s.lower() , $0)
}
 ...
 */
public enum NormalizerLifecycle {
    /**
